...
export default function VehiclesPage() {
  ...
  const ADMIN_CATALOG = [
    { id: 1, name: 'Toyota Avanza 1.3 G', type: 'Mobil', fuel: 'Bensin', efficiency: 14.5 },
    { id: 2, name: 'Toyota Innova Zenix Hybrid', type: 'Mobil', fuel: 'Bensin', efficiency: 22.0, isHybrid: true },
    { id: 3, name: 'Honda Brio Satya', type: 'Mobil', fuel: 'Bensin', efficiency: 20.3 },
    { id: 4, name: 'Mitsubishi Pajero Sport', type: 'Mobil', fuel: 'Solar', efficiency: 11.2 },
    { id: 5, name: 'Honda Vario 160', type: 'Motor', fuel: 'Bensin', efficiency: 46.9 },
    { id: 6, name: 'Tesla Model 3', type: 'Mobil', fuel: 'Listrik', efficiency: 6.5 },
  ];
  ...
  const handleSubmit = (e) => {
    ...
    setIsFormOpen(false);
    setInputValue('');
    setFormData({ name: '', type: 'Mobil', fuel: 'Bensin', efficiency: '', isCustom: false });
  };
    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Tambahkan kendaraan untuk mulai mencatat emisi.</p>
        </div>
        ...
        <Card className="bg-white border-slate-200 shadow-lg overflow-visible relative z-20">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="relative" ref={dropdownRef}>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-2 tracking-wider">Nama Kendaraan</label>
              ...
              {showDropdown && (
                <div className="absolute top-full left-0 w-full bg-white border border-slate-200 rounded-lg shadow-xl mt-2 max-h-60 overflow-y-auto z-50 animate-fade-in-down">
                  ...
                  {filteredOptions.length > 0 && (
                    <div className="py-2">
                      <p className="px-4 py-1 text-[10px] font-bold text-slate-400 uppercase">Katalog Tersedia</p>
                      {filteredOptions.map((item) => (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-slate-500">{item.type} • {item.fuel}</p>
                          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in-up pt-2">
                ...
                {formData.isHybrid && (
                  <div className="md:col-span-3 bg-emerald-50 border border-emerald-100 p-3 rounded-lg flex items-center gap-3 text-sm text-emerald-800">
                    ...
                    <b>Hybrid System Detected:</b> Efisiensi otomatis disesuaikan (Listrik + BBM).
                  </div>
                )}
                ...
                <div>
                  ...
                  <select 
                    className={`w-full p-2 rounded border border-slate-300 bg-white focus:ring-2 focus:ring-slate-800 outline-none ${!formData.isCustom && 'bg-slate-100 text-slate-500 pointer-events-none'}`}
                    value={formData.type}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Bahan Bakar</label>
                  <select 
                     className={`w-full p-2 rounded border border-slate-300 bg-white focus:ring-2 focus:ring-slate-800 outline-none ${!formData.isCustom && 'bg-slate-100 text-slate-500 pointer-events-none'}`}
                    value={formData.fuel}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-2">
                    Efisiensi (km/L)
                    {!formData.isCustom && <span className="ml-1 text-[10px] font-normal text-slate-400">(Boleh disesuaikan)</span>}
                  </label>
                  <input 
                ...
                <div className="md:col-span-3 flex justify-end pt-4 border-t border-slate-100">
                  <Button type="submit" variant="primary">Simpan ke Garasi</Button>
                </div>
              </div>
            )}
          </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {myVehicles.map((v) => (
          <div key={v.id} className="group bg-white border border-slate-200 rounded-xl p-6 hover:shadow-lg transition-all relative">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-4">
                 <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${v.isHybrid ? 'bg-emerald-100' : 'bg-slate-100'}`}>
                   {v.type === 'Motor' ? '🛵' : '🚘'}
                 </div>
                 <div>
                   <h3 className="font-bold text-slate-800">{v.name}</h3>
                   <div className="flex gap-2 mt-1">
                     <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded text-slate-500 font-bold uppercase border border-slate-200">{v.fuel}</span>
                     {v.isCustom && <span className="text-[10px] bg-amber-50 px-2 py-0.5 rounded text-amber-600 font-bold uppercase border border-amber-200">Custom Data</span>}
                   </div>
                 </div>
              </div>
            ...
            <div className="flex items-center gap-2 bg-slate-50 p-3 rounded-lg border border-slate-100">
               <span className="text-xs font-bold text-slate-400 uppercase">Efisiensi:</span>
